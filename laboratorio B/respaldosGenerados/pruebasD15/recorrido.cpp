...
int verificar(int *disponibles, int m, sala *salas, charla *charlas, int hall, int tiempoini, int tiempofin);
void backtracking(charla* charlas, int n, sala* salas, int m, int *solucion, int i, int *tupla, int *disponibles);
...
int main(){
	...
	printf("Antes de ordenar por fecha fin \n");
	for(int i=0; i < n; i++)
	{
		printf("indice %d charlas[%d]=%d \n", indices[i], i, charlas[i].fin);
	}

	SelectionSort(charlas, indices, n);

	printf("Ordenadas por fecha fin \n");
	...
	*/
	...
	backtracking(charlas, n, salas, m, solucion, 0, tupla, disponibles);
			...
			for(int j=i; j<n; j++){
				...
				for(int xi=0; xi < m; xi++)
				{
					...
					//if ( (xi < 0) || disponible(charlas[j].inicio,  charlas[j].fin, salas[xi].inicio, salas[xi].fin )){
							...
							if(j<n && verificar(disponibles, m, salas, charlas, xi, charlas[i].inicio, charlas[i].fin)){
								//tupla[j]=xi;
								tupla[j]= xi;
								//if(xi>=0){
...
//
			...
			if( esMejor(tupla[n], solucion[n])){
				copiar(tupla,solucion, n);
								...
								backtracking(charlas, n, salas, m, solucion, j+1, tupla, disponibles);
								//}
								//if(xi>=0){
									tupla[n]-=charlas[j].asistentes;
									tupla[n+1]+=charlas[j].asistentes;
...
int verificar(int *disponibles, int m, sala *salas, charla *charlas, int hall, int tiempoini, int tiempofin){
		...
		if(disponibles[hall]==-1 && disponible(tiempoini, tiempofin, salas[hall].inicio, salas[hall].fin)){
			disponibles[hall]=tiempofin;
			return true;
		}else if(disponibles[hall] <= tiempoini ){
			disponibles[hall]=tiempofin;
			return true;
		}
