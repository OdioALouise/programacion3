...
bool verificaRest(int *tupla, int i, charla* charlas, int n);
...
void backtracking(charla* charlas, int n, sala* salas, int m, int *solucion, int i, int *tupla, int parche)
{
	...
	if( verificaRest(tupla, i, charlas, n) )
	{
...
bool esMejor(int *tupla, int *solucion, charla* charlas, int n){
	...
	printf("TUPLAS %d SOLUCION %d", asistentes_tupla, asistentes_solucion);
	...
	for(int j=0; j<n; j++)
	{
		...
		//printf("Valores de tupla tupla[%d]=%d  tupla[%d]=%d \n", j, tupla[j], i-1, tupla[i-1]);	
			...
			//printf("\n");
				...
				if(k!=j && tupla[j] != -1 && tupla[k]==tupla[j]){
					...
					if( ! ( ( charlas[k].fin <=  charlas[j].inicio) || ( charlas[j].fin <=  charlas[k].inicio) ) ){
